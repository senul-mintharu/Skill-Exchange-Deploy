  ...
  <div className="ui-card p-4 sm:p-5">
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1 space-y-2">
        <div className="ui-skeleton h-5 w-2/5" />
          ...
          <>
            ...
            <ul className="overflow-hidden rounded-panel border border-line bg-white shadow-card divide-y divide-line">
              {filteredWorkers.map((worker) => (
                <li key={worker.id} className="p-4 sm:p-5">
                  <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
                    <div className="relative h-12 w-12 shrink-0 self-center sm:h-14 sm:w-14 sm:self-auto">
                      <div
                        className="flex h-full w-full items-center justify-center overflow-hidden rounded-full text-lg font-bold text-white shadow-brand sm:text-xl"
                      ...
                      {worker.verificationStatus === 'APPROVED' ? (
                        <span className="absolute -bottom-1 -right-1 z-20 flex h-5 w-5 items-center justify-center rounded-full border-2 border-white bg-white shadow-soft" title="Verified Worker">
                          <span className="material-icons text-xs text-brand-700">verified</span>
                        </span>
                      ) : null}
                    </div>
                        ...
                        <div className="min-w-0 space-y-1">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className="rounded-chip bg-slate-200 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-slate-700">
                              {worker.verificationStatus === 'APPROVED' ? 'Verified' : 'Unverified'}
                            </span>
                          </div>
                          ...
                          <p className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-800">
                        ...
                        <div className="flex items-start justify-between gap-3 sm:justify-start">
                          ...
                          <div className="text-right">
                            <p className="text-lg font-extrabold tracking-tight text-ink">
                              {worker.hourlyRate > 0 ? `Rs. ${worker.hourlyRate.toLocaleString()}` : 'N/A'}
                            </p>
                            <p className="ui-stat-label">{worker.hourlyRate > 0 ? 'Hourly Rate' : 'Rate'}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 flex flex-col gap-3 border-t border-line/70 pt-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex flex-wrap items-center gap-4">
                          <p className="text-sm text-ink-muted">
                            District: <span className="font-semibold text-ink">{worker.district || 'Not set'}</span>
                          </p>
                          <p className="text-sm text-ink-muted">
                            Completed: <span className="font-semibold text-ink">{jobsCompletedForWorker(worker) !== null ? jobsCompletedForWorker(worker) : '—'}</span>
                          </p>
                        </div>
                        <Link to={`/workers/${worker.id}`} className="ui-button-primary w-full justify-center sm:w-auto sm:px-5">
                          View Profile
